...
本工具提供两种使用方式：命令行方式和 Web 界面方式。

### 方式一：命令行方式 ⌨️

1. **准备数据文件** 📊

   - 将基准销售毛利分析表文件放入 `my` 文件夹
   - 将比较销售毛利分析表文件放入 `from` 文件夹
   - 确保两个文件的名称均为 `销售毛利分析表.xlsx`
2. **运行脚本** ▶️
   ...
   ```
3. **自定义配置** ⚙️
   如需自定义文件路径，可在脚本的 `if __name__ == "__main__":` 部分修改以下参数：
...
├── .gitignore              # Git忽略文件配置
├── README.md               # 项目说明文档 📖
└── compare_excel_销售毛利分析表.py  # 命令行主程序文件 🐍
```
