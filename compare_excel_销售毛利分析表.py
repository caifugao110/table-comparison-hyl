        ...
        for key, row_baseline in all_baseline_keys.items():
            if key not in all_compare_keys:
                ...
                for col in range(1, baseline_max_col + 1):
                    ws_baseline.cell(row=row_baseline, column=col).fill = fill_deleted
                changes_count += 1
                deleted_rows += 1
        print(f"已标记 {deleted_rows} 行删除（红色）")
        ...
        for key, row_compare in all_compare_keys.items():
            if key not in all_baseline_keys:
                ...
                for col in range(1, compare_max_col + 1):
                    ws_compare.cell(row=row_compare, column=col).fill = fill_added
                changes_count += 1
                added_rows += 1
        print(f"已标记 {added_rows} 行新增（绿色）")
    else:
        # 使用简单的行匹配来标记新增和删除行
        print("\n使用简单匹配标记新增和删除行...")
